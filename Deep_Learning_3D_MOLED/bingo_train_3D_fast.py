...
from tools.data_loader_mat import get_loader_fast
                ...
                if total_iters%config.step == 0:
                  with torch.no_grad():
                    ...
                    for i, (brain_images, _) in enumerate(brain_batch):
                        ...
                        save_torch_result(SR_brain[:, 0:1, :, :,64], save_dir,
                                          format='png', cmap='jet', norm=False, crange=[0,0.2])

                        # save_dir = os.path.join(save_inter_result, 'inter_qsm_' + str(total_iters) + '_brain')
                        # save_torch_result(SR_brain[:, 0:1, :, :, 64], save_dir,
                        #                   format='png', cmap='gray', norm=False, crange=[-0.1, 0.1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_updata', type=int, default=120000)  # epoch num for lr updata
    ...
    config.name = 'OLED3D_t2star_R2AttUNet'
    main(config)
