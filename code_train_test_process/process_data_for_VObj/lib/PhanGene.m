...
for num = 1:1:Number
    ...
    RandCoord3 = round(rand(1,3)*size3); % Set z coordinate

    CoorVec(num,1) = num; % Loop number
    CoorVec(num,2:4) = [RandCoord1(1),RandCoord2(1),RandCoord3(1)]; % x1,y1,z1
    Coor_X = CoorVec(num,2); Coor_Y = CoorVec(num,3); Coor_Z = CoorVec(num,4); % Cube Initial Coordinate
    SusVal = (rand(1,3)*1000-600);  % rand(0,1) refers to uniform distribution between 0~1
    Susceptibility(num,1) = SusVal(1);%  Suceptibility Value, Sphere

    % RandCoordS = round(rand(1,3)*size1);
    CoorVecS(num,1) = num; % Loop number
    CoorVecS(num,2:4) = [RandCoord1(2),RandCoord2(2),RandCoord3(2)]; % x2,y2,z2
    ...
    CoorVecCL(num,2:4) = [RandCoord1(3),RandCoord2(3),RandCoord3(3)]; % x3,y3,z3
